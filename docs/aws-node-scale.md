...
Please wait a few moments for all the replicas to have the status `running`. After the `monitor` stack is up and running, we can move on to deploying node exporters.
...
After a few moments, the alert will fire and trigger `scaler` to scale worker nodes up by one. We confirm that there is now five nodes by running:
...
After a few moments, the alert will fire and trigger `scaler` to scale worker nodes down by one. We confirm that there is now four nodes by running:
